...
<template>
    ...
    <transition :duration="1600">
        <Chat ref="chat" v-if="showChat" v-show="!showSetting" :showChat="showChat" @signOut="sign_out"
            @show-setting="showSetting = true" />
    </transition>
    <transition :duration="1500">
        <Setting v-if="showSetting" @back="showSetting = false" @update="updateUserInfo"></Setting>
    </transition>
</template>
