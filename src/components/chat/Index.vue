...
async function init() {
  axios({
    ...
    socket = createSocket(
      userInfo as Ref<UserInfo>,
      () => {
        inputDisabled.value = false;
      },
      () => scroll2Bottom("auto"),
      () => scroll2Bottom("smooth")
    );
  });
}
...
<template>
  <div>
            ...
            <div class="chat_group" v-for="group in msgList" :key="group.time">
              ...
              <transition-group tag="ul" name="list" :key="group.time">
                <li v-for="item in group.msgList" :key="item.msgId" :class="{ 'is-self': item.isSelf }">
                  <div class="head" :style="item.avatar ? `background-image:url('${item.avatar}')` : ''"></div>
    ...
    <MyButton
      ...
      :line_height="6"
      line_gap="0.2rem"
      line_background="#210b35"
      change_background="rgb(197, 9, 239)"
      v-model="showSlider"
    >
    </MyButton>
